...
def GetDatas(dict_cal : dict, debug : bool = False) -> tuple[list[str], dict[str, object]]:
    #* Retornar datas cadastradas

    datas = []
    erros = {}

    for instancia in dict_cal:
        if isinstance(dict_cal[instancia], dict):
            for data in dict_cal[instancia]:
                try:
                    dia : str = data[:2]
                    mes : str = data[3:5]
                    ano : str = data[6:]
                    ...
                    and 2000 < int(ano):
                        ...
                        erros[instancia][data] = dict_cal[instancia][data]

                        if debug:
                            print(f'\nData invalida em: `{instancia}` -> `{data} : {dict_cal[instancia][data]}`')
                    
                except:
                    erros[instancia] = {data : dict_cal[instancia][data]} #! salva erros em indentação no nível de datas
                    #? salvando somente último erro
                    
                    if debug:                        
                        print(f'\nFormato invalido em: `{instancia}` -> `{data} : {dict_cal[instancia][data]}`\n')

                        if isinstance(dict_cal[instancia], dict):
                            PrintarRecursivo(dict_cal[instancia][data])
        
        else:
            erros[instancia] = dict_cal[instancia] #! salva erros em indentação no nível de chaves-primárias

            if debug:
                print(f'\n{instancia} => não é Dicionário ({instancia} : {dict_cal[instancia]} -> {type(dict_cal[instancia])})')
    ...
    listas = (sorted(datas, key=lambda data: Str2Data(data)), erros)
...
def Add2JSON(pathJSON : str, dict_cal : dict, data : str, instancia : str = "Default", notas : str = "Default", debug : bool = False):
    ...
    dict_cal["paradas"][data] = {"instância" : instancia, "notas" : notas}

    if debug:
        print(f'adicionado -> dict_cal["paradas"] = "{data}" : {{"instância" : {instancia}, "notas" : {notas}}}')
...
def RemoveFromJSON(pathJSON : str, dict_cal : dict, data : str, debug : bool = False):
    ...
    for chave in dict_cal:
        ...
        if isinstance(dict_cal[chave], dict):
            if data in dict_cal[chave]:
                dados = dict_cal[chave][data]

                if dados == dict_cal[chave].pop(data, "data não existente"):
                    if debug:
                        print(f"Removido: {chave}:{'{'}{data} : {dados}{'}'}")
...
def CalcularDiaOpt(dict_cal : dict, debug : bool = False) -> tuple[datetime.datetime, int]:
    ...
    datas_programadas = GetDatas(dict_cal, debug=debug)[0]

    atualidade = datetime.datetime.now()
    atualidade = atualidade.strftime("%d/%m/%Y")  #! convertendo para string no formato dia/mês/ano

    datas_programadas.insert(0, atualidade)
    datas_programadas = [Str2Data(data) for data in datas_programadas if Str2Data(data) > Str2Data(atualidade)]
